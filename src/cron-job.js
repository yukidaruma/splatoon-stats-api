...
const fetchStageRotations = forceFetch => new Promise((resolve, reject) => {
  ...
  db('league_schedules').where('start_time', '>=', dateToSqlTimestamp(new Date())).then((rows) => {
    if (forceFetch || rows.length < 6) { // When there's less than 6 future schedules
      fetch('https://splatoon2.ink/data/schedules.json', { headers: { 'User-Agent': config.THIRDPARTY_API_USERAGENT } })
        .then(res => res.json())
        .then((schedule) => {
          const tasks = schedule.league.map((league) => {
            ...
            return db.raw('INSERT INTO league_schedules (start_time, rule_id, stage_ids) VALUES (to_timestamp(?), ?, ?) ON CONFLICT (start_time) DO NOTHING',
  ...
  getSplatnetApi(`league_match_ranking/${leagueId}/ALL`) // ALL = global ranking
    .then((ranking) => {
      const tasks = flat(ranking.rankings.map((group) => {
        ...
        return group.tag_members.map(member => db.raw('INSERT INTO league_rankings (start_time, group_type, group_id, player_id, weapon_id, rank, rating) VALUES (to_timestamp(?), ?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING',
          [ranking.start_time, groupType, group.tag_id, member.principal_id, member.weapon.id, group.rank, group.point]));
      }));
