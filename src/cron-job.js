...
const cacheImageFromSplatoon2Ink = async (remotePath, id) => {
  ...
  const cachePath = 'cache' + remotePath.replace(/\/[\da-f]+\.([a-zA-Z]+)$/, (match, ext) => `/${id}.${ext}`);
  const cacheDir = cachePath.substring(0, cachePath.lastIndexOf('/'));
 ...
 */
const insertKnownNames = (playerId, playerName, lastUsed) =>
  db.raw(
    `INSERT
 ...
 */
const fetchStageRotations = (forceFetch) =>
  new Promise((resolve, reject) => {
    // TODO: You can skip running query when forceFetch is true.
    db('league_schedules')
      .where('start_time', '>=', 'now()')
      .then((rows) => {
        if (forceFetch || rows.length < 6) {
          // When there's less than 6 future schedules
            ...
            .then((schedule) => {
              ...
              INSERT
                ...
                ON CONFLICT (start_time) DO NOTHING`,
                  [league.start_time, findRuleId(league.rule.key), stageIds],
                );
              });
    ...
    (async function () {
      ...
      for (const rule of rankedRules) {
        ...
        for (const page of [1, 2, 3, 4, 5]) {
          ...
          const queries = ranking.top_rankings
            .map((player) => [
              db.raw(
                `
            INSERT
...
const fetchSplatfestSchedules = () => {
  fetch('https://splatoon2.ink/data/festivals.json', { 'User-Agent': config.THIRDPARTY_API_USERAGENT })
    .then((res) => res.json())
    .then((regions) => {
      Object.entries(regions).forEach(([region, splatfests]) => {
        const queries = splatfests.festivals.map((splatfest) => {
          const colors = Object.entries(splatfest.colors)
          ...
          return db.raw(
            ...
            [region, splatfest.festival_id, splatfest.times.start, splatfest.times.end, colors, teamNames],
          );
        });
  ...
  return db.transaction((trx) => {
    fetch(`https://splatoon2.ink/data/festivals/${region}-${splatfestId}-rankings.json`, {
      ...
      .then((rankings) => {
        ...
        ['alpha', 'bravo'].forEach((key, teamId) => {
          // As teamId, use 0 for alpha and 1 for bravo.
          ...
          ranking.forEach((player) => {
            ...
            queries.push(
              ...
              INSERT
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT DO NOTHING`,
                  [region, splatfestId, teamId, playerId, player.info.weapon.id, player.order, player.score],
                )
                .transacting(trx),
            );
          });
